...
public class Auction_EndList extends HttpServlet 
{
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException 
	{
		...
		int totalRecordCount = 
			dao.EndListgetTotalRecordCount(param);
...
/*		int pageSize = 
		...
		*/
		 ...
		 * 가상번호 계산을 위한 추가
		 */
		param.put("mode", mode);//마감된리스트 보여주기
		...
		List<AuctionDTO> lists = dao.selectEndList(param);
		
		//페이지 처리를 위한 문자열 생성
		String pagingImg = PagingUtil.pagingImgServlet(
			totalRecordCount,pageSize,
			blockPage, nowPage, 
			"../Yagaja/Auction_EndList?mode="+mode+"&"+addQueryString);
		...
		req.getRequestDispatcher("/auction/auction_endlist.jsp");
